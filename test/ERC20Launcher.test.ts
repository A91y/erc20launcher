...
describe("ERC20Launcher", function () {
  ...
  describe("Minting", function () {
    it("Should allow owner to mint tokens", async function () {
      ...
      await erc20Launcher.mint(otherAccount.address, ethers.utils.parseUnits("1000", 0));
    ...
    it("Should stop minting after max supply is reached", async function () {
      const { owner, erc20Launcher } = await loadFixture(deployFixture);
      const initialSupply = ethers.utils.parseUnits("1000000", 18);
      await erc20Launcher.mint(owner.address, ethers.utils.parseUnits("900000", 0));
  ...
  describe("Transfers", function () {
    ...
    it("Should fail if sender does not have enough balance", async function () {
      ...
      await expect(erc20Launcher.connect(otherAccount).transfer(owner.address, ethers.utils.parseUnits("1000", 18)))
        .to.be.revertedWithCustomError(erc20Launcher, "ERC20InsufficientBalance");
    });
    ...
    it("Should update balances after transfers", async function () {
      ...
      expect(await erc20Launcher.balanceOf(owner.address)).to.equal(ethers.utils.parseUnits("1000000", 18).sub(ethers.utils.parseUnits("500", 18)));
    });
  });
});
